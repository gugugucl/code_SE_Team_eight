...
private slots:
    void on_btnRefresh_clicked();//从数据库中查询待办事项并根据不同的排序方式显示它们，同时提供一个按钮用于切换已完成任务的显示。

    void openSettingPanel(QString Title, QString Deadline); //打开设置窗口

    void groupSwitch(QString group); //分组切换

    void on_editTitle_editingFinished(); //待办事项创建 点回车
...
private:
    ...
    QSqlDatabase db=QSqlDatabase::addDatabase("QMYSQL");  // 使用 MySQL 驱动; //数据库
