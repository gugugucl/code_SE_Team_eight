...
void QMessageNotification::mousePressEvent(QMouseEvent *event)//按下鼠标左键
{
...
void QMessageNotification::mouseMoveEvent(QMouseEvent *event)//鼠标处于拖动状态
{
    if(m_bDrag)
    {
...
void QMessageNotification::mouseReleaseEvent(QMouseEvent *event)//松开鼠标左键
{
    if(event->button() == Qt::LeftButton)
    {
        m_bDrag = false;
...
void QMessageNotification::paintEvent(QPaintEvent *event)//绘制窗口，颜色、形状、字体等等在这里处理
{
    Q_UNUSED(event);
...
void QMessageNotification::on_CloseOK_clicked()//点击“我知道了”
{
    close();
}
