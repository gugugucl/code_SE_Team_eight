    ...
    ui(new Ui::GToDoitem)
{
    ...
    QAction *addDir = popMenu->addAction(QIcon(":/image/write.png"), tr("修改该记录"));//编辑按钮
    connect(addDir, &QAction::triggered, this, [=](){
        on_lbTitle_clicked();
    });
    QAction *addTemplate = popMenu->addAction(QIcon(":/image/del.png"), tr("删除该记录"));//删除按钮
    ...
    QAction *addDownload = popMenu->addAction(QIcon(":/image/download.png"), tr("下载该记录"));//下载按钮
...
{
    if(popMenu){
        popMenu->exec(QCursor::pos());//当鼠标指在这里时，会有阴影表示选中提示
    }
}
...
void GToDoitem::setTitle(QString Title, int priority)//设置优先级
{
...
void GToDoitem::setDDL(QString date)//设置截止日期
{
...
void GToDoitem::setRepeat(int Repeat)//设置重复次数
{
    switch (Repeat) {
    case 1:
...
void GToDoitem::setCrontab(int Crontab)//任务类型
{
    ...
    case 2:
        ...
        ui->lbCrontab->setToolTip("定时清理");//鼠标悬浮在这上面就显示"定时清理"
        ui->lbCrontab->setText(tr("清理"));//标签上面默认显式的内容为'清理'
        break;
    case 3:
...
void GToDoitem::setRemind(QString _datetime)//设置截止日期时间
{
...
void GToDoitem::setFinishDate(QString date)//已完成的任务的Ddl特殊处理一下
{
    ui->lbDdl->setText(GQStringToQDate::QStringFromat(date)+tr("完成"));
}
void GToDoitem::hideBtn()//对于已完成的任务，确认完成键就隐藏起来
{
    //ui->btnAlter->hide();
    ui->btnFinish->hide();
}
...
void GToDoitem::on_btnFinish_clicked()//点击√确认键，保存相关信息，更新数据库
{
    QDate date;
    date=date.currentDate();
    QSqlQuery query;
...
void GToDoitem::on_btnAlter_clicked()//发送编辑信号
{
    emit showSettingPanel(ui->lbTitle->text(),m_ddl);
}

void GToDoitem::on_lbIcon_clicked()//点击图标也发送编辑信号
{

    emit showSettingPanel(ui->lbTitle->text(),m_ddl);
}

void GToDoitem::on_lbTitle_clicked()//点击任务标题也发送编辑信号
{
    emit showSettingPanel(ui->lbTitle->text(),m_ddl);
}


void GToDoitem::on_pushButton_download_clicked()//下载
{
...
void GToDoitem::on_pushButton_del_clicked()//点击删除按钮
{
    QMessageBox mm(QMessageBox::Warning,QString(tr("警告")),QString(tr("你确定要删除吗？")),QMessageBox::Yes| QMessageBox::No);
    mm.setButtonText(QMessageBox::Yes,QString(tr("确认")));
    mm.button(QMessageBox::No)->setText(tr("取消"));
